...
init_word_char (re_dfa_t *dfa)
{
  ...
  if (BE (dfa->map_notascii == 0, 1))
      ...
      if (sizeof (dfa->word_char[0]) == 8)
	{
	  ...
	  dfa->word_char[0] = (bitset_word_t)wc0;
	  dfa->word_char[1] = (bitset_word_t)wc1;
	  i = 2;
	}
  ...
  for (; i < BITSET_WORDS; ++i)
    for (size_t j = 0; j < BITSET_WORD_BITS; ++j, ++ch)
      if (isalnum (ch) || ch == '_')
...
analyze (regex_t *preg)
{
  ...
  if (dfa->subexp_map != NULL)
	...
	dfa->subexp_map[i] = i;
      ...
      for (i = 0; i < preg->re_nsub; i++)
	if (dfa->subexp_map[i] < 0 || ((size_t)dfa->subexp_map[i]) != i)
	  break;
      if (i == preg->re_nsub)
	{
	  free (dfa->subexp_map);
  ...
  for (src = 0; src < dfa->nodes_len; ++src)
    {
      ...
      idx = 0;
      if (dfa->eclosures[src].nelem > 0)
      for (; idx < (size_t)dfa->eclosures[src].nelem; ++idx)
	{
